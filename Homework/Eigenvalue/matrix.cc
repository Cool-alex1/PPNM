...
// If the numbers should be above eachother
// void matrix::print(std::string s) const {
// 	std::cout << s << "";
// 	std::string front = std::string(s.length()+0, ' ');
// 	int largeVal = 1;
//     for(int i = 0; i < nRow ; ++i) for(int j = 0; j < nCol ; ++j) largeVal = std::max((int)(std::to_string(mat[i*nCol + j]).size()), largeVal);  
// 	for (int i=0; i < nRow; i++){
// 		if(i == 0) std::cout << "[";
// 		else std::cout << front << "[";
...
// 		for (int j=0; j < nCol; j++){
// 			if(j != 0) std::cout << ",";
// 			std::cout << std::to_string(mat[i*nCol + j]) << std::string(largeVal-std::to_string(mat[i*nCol + j]).size()+1, ' ') <<  mat[i*nCol + j];
// 		}
